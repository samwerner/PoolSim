...
std::shared_ptr<MiningPool> MiningPool::create(
  ...
  return create(difficulty, uncle_prob, std::move(reward_scheme), SystemRandom::get_instance());
}
...
std::shared_ptr<MiningPool> MiningPool::create(
  ...
  auto mining_pool = std::shared_ptr<MiningPool>(new MiningPool(difficulty, uncle_prob, random));
...
MiningPool::MiningPool(uint64_t _difficulty, double _uncle_prob, std::shared_ptr<Random> _random)
  : difficulty(_difficulty), uncle_prob(_uncle_prob), random(_random) {}
